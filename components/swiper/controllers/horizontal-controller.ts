...
export class HorizontalController extends SwiperController {
  ...
  getTransformByProgress ({ progressInSlide }: { progressInSlide: number }) {
    ...
    if (loop) {
      ...
      slideIndexList.push(...Array.from({ length: this.slideCount }, (_, i) => i));
    }

    for (let i = 0; i < this.slideCount; i++) {
      const slideIndex = slideIndexList[i];

      positions[slideIndex][0] = (i - currentSlideInListIndex) * slideDistance + progressInSlide * slideDistance;
    }

    this.slideIndexList = slideIndexList;
