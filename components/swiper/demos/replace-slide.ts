...
import type { SwiperData } from '@galacean/effects-components';
...
(async () => {
  try {
    ...
    const compositions = await player.loadScene([mainJSON, slideJSON[initSlideIndex]]);
    ...
    const swiperManager = new SwiperManager({
      ...
      slideCompositions: [{
        ...
        index: initSlideIndex, // 当前幻灯片的索引
        url: slideJSON[initSlideIndex], // 当前合成的url（用于区分不同的幻灯片内容）
      }],
      player: player,
      options: {
        initSlideIndex,
    ...
    const totalSlideCount = 3;

    await swiperManager.setUpLoading(totalSlideCount, defaultLoadingJSON);
    ...
    setTimeout(() => {
      const newSlidesList = [{
        ...
        url: 'https://mdn.alipayobjects.com/mars/afts/file/A*VNPBSadK2ekAAAAAQaAAAAgAelB4AQ',
      ...
      console.info('幻灯片发生更新');
      void swiperManager.replaceSlides(newSlidesList, newSwiperOptions);
    }, 2000);
